...
def bind_material(prim: Usd.Prim, mat_target: Union[str, Sdf.Path, UsdShade.Material]) -> None:
    """Binds a UsdShade.Material to a USD Prim."""
    if isinstance(mat_target, UsdShade.Material):
        material = mat_target
    else:
        stage = prim.GetStage()
        path = Sdf.Path(mat_target) if isinstance(mat_target, str) else mat_target
...
def add_distant_light(
    ...
    xformable.AddRotateXYZOp().Set(Gf.Vec3f(*rotation_xyz))
    return light
...
def add_ground_plane(
    ...
    size: float = 2000.0,
    material_path: Optional[Union[str, Sdf.Path, UsdShade.Material]] = None
) -> UsdGeom.Plane:
    ...
    if material_path is not None:
        bind_material(plane.GetPrim(), material_path)
